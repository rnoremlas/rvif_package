...
## *plot.cv_vif* function

The *plot.cv_vif* function is intended to facilitate the interpretation of the results provided by the *cv_vif* function. For this reason, the input of this function (first argument) is the output from the *cv_vif* function.
...
Another equivalent way to obtain this representation is:
...
Please note that horizontal and vertical lines do not appear in the above graphical representations because the environment considers their coordinates to be outside the range of the graph; however, they are displayed correctly in the package help.

When the *plot.cv_vif* function is applied to the simulation of the second example of the *cv_vif* function, it can be observed that the second variable causes non-essential multicollinearity while the third and fourth variables cause essential multicollinearity:
...
On the other hand, in the event that the *cv_vif* function returns an error, this error is reproduced when attempting to display its output:
